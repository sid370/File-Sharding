...
app.post("/cli/file/:path", (req, res, next) => {
  ...
  cmd.stdout.on("data", async (data) => {
    ...
    for (var i = 0; i < 3; i++) {
      ...
      temp_uid = await random.randomisation(uniqueName.split(" ").join(""));
...
app.get("/genFile/:uid", async (req, res, next) => {
  resp = [];
  //uid = random.derandomisation(req.params.uid);
  //await sleep(2);
  //resp[0] = req.params.uid.slice(0, 64);
  //resp[1] = req.params.uid.slice(64, 128);
  //resp[2] = req.params.uid.slice(128, 192);

  uid = await random.derandomisation(req.params.uid)
  
  resp[0] = uid.slice(0, 64);
  resp[1] = uid.slice(64, 128);
  resp[2] = uid.slice(128, 192);
